...
Это REST API приложение предназначено для автоматизации процесса импорта данных из внешних источников (CSV файлов) в базу данных. Основная функция - загрузка справочников регионов, районов, городов и организаций с их контактной информацией.
...
Создайте базу данных MySQL и настройте параметры подключения PDO в файле `config/db.php`:
```php
return [
      'host' => '127.0.0.1',
      'dbname' => 'csv',
...
Для локального запуска достаточно запустить:
```bash
...
- `file` (обязательный) - CSV файл с данными о компаниях
...
- `vkontakte` - ссылка на ВКонтакте
- `odnoklassniki` - ссылка на Одноклассники
